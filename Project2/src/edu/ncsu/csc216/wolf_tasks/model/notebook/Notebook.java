...
public class Notebook {
	...
	public String[] getTaskListsNames() {
		if(taskLists.size() != 0) {
			...
			return new String[] {"Active Tasks"};
		}
	}
	...
	private void getActiveTaskList() {
		AbstractTaskList temp = new TaskList("Active Tasks", activeTaskList.getCompletedCount());
		for(int i = 0; i < taskLists.size(); i++) {
	...
	public void editTaskList(String name) {
		...
		if ("Active Tasks".equalsIgnoreCase(name)) {
	        throw new IllegalArgumentException("Invalid name.");
	    }
	...
	public void editTask(int index, String name, String description, boolean isRecurring, boolean isActive) {
		if(currentTaskList instanceof TaskList) {
			currentTaskList.getTasks().get(index).setTaskName(name);
